...
public func ~<A, B, C: Fixed>(_ f: @escaping (A, B) -> C.Unfixed, _ value: A) -> (B) -> C {
    { b in C.fix(f(value, b))}
}

public func ~<A, B, C: Fixed>(_ f: @escaping (A, B) -> C.Unfixed, _ values: (PlaceHolder, B)) -> (A) -> C {
    { a in C.fix(f(a, values.1))}
}

public func ~<A, B, C: Fixed>(_ f: @escaping (A, B) -> C.Unfixed, _ values: (A, PlaceHolder)) -> (B) -> C {
    { b in C.fix(f(values.0, b))}
}
